...
# def random_resized_crop_with_boxes(
#     images,               # (T, C, H, W)
#     target_height,
#     target_width,
#     boxes=None,           # (T,4) in xyxy pixel coords
#     scale=(0.3, 1.0),
...
# ):
#     """
#     RandomResizedCrop, but:
#       - with prob p_box_crop: crop is sampled around the (middle-frame) bbox (guaranteed to include it)
#       - else: fully random crop
#     Returns:
#       images_out: (T, C, target_height, target_width)
#       boxes_out: (T,4) in pixel coords of the resized output
#     """
#     T, C, H, W = images.shape

#     # --- choose crop params (i, j, h, w) ---
#     use_box = boxes is not None and random.random() < p_box_crop
#     if use_box:
#         b = np.asarray(boxes, dtype=np.float32)
#         x1, y1, x2, y2 = b[len(b) // 2]  # middle frame
...
#         # sample crop size as a multiple of bbox size (adds context)
#         m = random.uniform(scale_mult[0], scale_mult[1])
#         crop_w = int(round(bw * m))
#         crop_h = int(round(bh * m))

#         # enforce sane bounds
#         crop_w = max(min_crop_size, min(W, crop_w))
#         crop_h = max(min_crop_size, min(H, crop_h))

#         # jitter crop center a bit (still keeps bbox inside)
#         cx += random.uniform(-center_jitter, center_jitter) * bw
#         cy += random.uniform(-center_jitter, center_jitter) * bh

#         j = int(round(cx - crop_w / 2))
#         i = int(round(cy - crop_h / 2))

#         # clamp to image
#         j = max(0, min(W - crop_w, j))
#         i = max(0, min(H - crop_h, i))

#         h, w = crop_h, crop_w
#     else:
#         # your original random sampler
#         i, j, h, w = _get_param_spatial_crop(scale, ratio, H, W)
...
#         b = np.asarray(boxes, dtype=np.float32).copy()  # (T,4)
...
#         # clip to crop window
#         b[:, [0, 2]] = np.clip(b[:, [0, 2]], 0.0, float(w - 1))
#         b[:, [1, 3]] = np.clip(b[:, [1, 3]], 0.0, float(h - 1))

#         # scale to resized output
...
def random_resized_crop_with_boxes(
    ...
    """
    Inception-style RandomResizedCrop, but bbox-aware.

    Returns:
      images_out: (T, C, target_height, target_width)
      boxes_out: same shape as input boxes, in pixel coords of the resized output
    """
    T, C, H, W = images.shape
    # ---- bbox-aware crop sampling ----
    p_box_crop = 0.9     # probability of forcing bbox-aware crop
    min_cover = 0.3        # require at least 30% of bbox area inside crop
    max_tries = 20

    def _inter_area(crop, box):
        x1 = max(crop[0], box[0]); y1 = max(crop[1], box[1])
        x2 = min(crop[2], box[2]); y2 = min(crop[3], box[3])
        return max(0.0, x2 - x1) * max(0.0, y2 - y1)

    if boxes is not None and random.random() < p_box_crop:
        b_np = np.asarray(boxes, dtype=np.float32)

        # boxes is (T,4) -> use middle frame
        ref_box = b_np[len(b_np) // 2]
        ...
        for _ in range(max_tries):
            i_try, j_try, h_try, w_try = _get_param_spatial_crop(scale, ratio, H, W)
            crop_box = (j_try, i_try, j_try + w_try, i_try + h_try)

            cover = _inter_area(crop_box, ref_box) / (box_area + 1e-6)
            if cover >= min_cover:
                i, j, h, w = i_try, j_try, h_try, w_try
                found = True
                break
    ...
    if boxes is None:
        ...
        # clip to crop window (same idea as clip_boxes_to_image) 
        b[:, [0, 2]] = np.clip(b[:, [0, 2]], 0.0, float(w - 1))
        b[:, [1, 3]] = np.clip(b[:, [1, 3]], 0.0, float(h - 1))

        # scale boxes to resized output
        sx = float(target_width) / float(w)
        sy = float(target_height) / float(h)
        b[:, [0, 2]] *= sx
        b[:, [1, 3]] *= sy

        boxes_out = b.reshape(orig_shape)
