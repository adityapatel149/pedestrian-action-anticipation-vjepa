...
class decode_videos_to_clips(wds.PipelineStage):
    ...
    def _get_bboxes(self, video_id: str, participant_id: str, indices: np.ndarray, buffer: np.ndarray) -> np.ndarray:
        ...
        # Keep bboxes in raw pixel coords here (x1,y1,x2,y2).
        # We clamp to the current frame size; normalization happens *after*
        # all spatial augmentations in the dataloader/transform.
    ...
    def run(self, src):
        ...
        for path in src:
            ...
            if video_id not in self.annotations:
                logging.warning(f"Video ID '{video_id}' not found in annotations; skipping path '{path}'")
                continue
            ...
            # Anticipation time (seconds) per sample.
...
# --- JAAD flat-directory helpers (no subfolders) ---
...
def _build_paths_and_annotations(
    ...
    """
    JAAD layout: all videos live directly under `base_path` with no subfolders.
    Tries several common extensions to resolve the file path.
    """
    ...
    for uv in unique_videos:
        fpath = None
        for ext in exts:
            candidate = os.path.join(base_path, uv + ext)
            if os.path.exists(candidate):
                fpath = candidate
                break
        if not fpath:
            logging.info(
                f"[JAAD flat] file path not found for video_id={uv} under base_path={base_path}; "
                f"tried exts={exts}"
            )
            continue
        video_paths.append(fpath)
        annotations[uv] = df[df["video_id"] == uv]
