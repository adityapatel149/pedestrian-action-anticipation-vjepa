...
"""
Evaluation / training loop adapted for PIE/JAAD with 1 head:
(cross).

Assumptions:
- Dataloader yields batches as: (video, bboxes, cross, anticipation_time)
- Classifier forward returns a dict with keys: {"cross": logits}
- filter_annotations() returns label_encoders for each of cross and
...
def main(args_eval, resume_preempt: bool = False):
    ...
    if args_opt.get("use_focal_loss", False):
        ...
        criterion = lambda x, y: softmax_focal_loss(x, y, gamma=2.0, alpha=alpha, reduction="mean")
...
def train_one_epoch(
    ...
    for itr in range(ipe):
        ...
        losses = []
        for o in outputs:
            lc = criterion(o["cross"], labels["cross"]) 
            losses.append(lc)
...
def test_inference(
    ...
    Notes:
    - encoders should be label_encoders from filter_annotations()
    - uses your existing _remap_labels(encoders, labels_raw, device)
    - avoids mixing ranks by writing rank-specific CSVs
    """
    os.makedirs(out_dir, exist_ok=True)

    model.eval()
    for c in classifiers:
        c.train(False)  # eval mode

    # open one CSV per sweep
    writers = []
    files = []
    ...
    try:
        for batch_idx, udata in enumerate(data_loader):
            with torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=use_bfloat16):
                clips = udata[0].to(device)
                bboxes = udata[1].to(device)
                labels_raw = {"cross": udata[2]}  # raw labels from loader
                anticipation_times = udata[-1].to(device)

                labels = _remap_labels(encoders, labels_raw, device)  # y_true (remapped)
                y_true = labels["cross"]  # shape: (B,)

                features = model(clips, anticipation_times)

                # run each sweep
                for j, clf in enumerate(classifiers):
                    out = clf(features, bboxes=bboxes)
                    logits = out["cross"]  # (B, C)
                    probs = F.softmax(logits, dim=-1)
                    pred = probs.argmax(dim=-1)

                    B, C = logits.shape

                    # write header once we know C
                    if not header_written[j]:
                        header = (
                            ["batch_idx", "sample_idx", "anticipation_time", "y_true", "pred"]
                            + [f"p{k}" for k in range(C)]
                            + [f"logit{k}" for k in range(C)]
                        )
                    ...
                    for i in range(B):
                        row = (
                            [batch_idx, i, float(anticipation_times[i].item()), int(y_true[i].item()), int(pred[i].item())]
                            + [float(probs[i, k].item()) for k in range(C)]
                            + [float(logits[i, k].item()) for k in range(C)]
                        )
