...
def build_bev_background(bev_config: BevConfig) -> np.ndarray:
    ...
    # for z_m in [5, 10, 15, 20]:
    #     py = int((1.0 - z_m / bev_config.max_range_m) * (bev_config.bev_size - 1))
    #     if 0 <= py < bev_config.bev_size:
    #         cv2.line(bev, (0, py), (bev_config.bev_size - 1, py), (55, 55, 55), 1)
    #         cv2.putText(
    #             bev,
    #             f"{z_m}m",
    #             (10, max(18, py - 6)),
    ...
    # cv2.line(
    #     bev,
    #     (bev_config.bev_size // 2, 0),
    #     (bev_config.bev_size // 2, bev_config.bev_size - 1),
    #     (70, 70, 70),
    #     1,
    #     cv2.LINE_AA,
    # )
...
def draw_depth_on_bev(
    ...
    x_m = pts_world[:, 0]
    z_m = pts_world[:, 1]
    z_m = np.clip(z_m, 0.0, bev_config.max_range_m)  # Ensure it doesn't go below 0
...
def render_bev(
    ...
    alpha: float = 0.1  # Smoothing factor for EMA (adjust as needed)
) -> np.ndarray:
    bev = build_bev_background(bev_config)

    # Track previous detections' positions (for smoothing)
    ...
    for det in detections:
        ...
        # Shift bev_y upward by 3 meters
        ...
        if pred is None:
            ...
            label = f"{dist_m:.1f}m r={risk_score:.2f}"
